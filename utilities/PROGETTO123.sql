...
-- * Generation date: Sun Jul  7 11:23:06 2019 
-- * LUN file: C:\Users\marco\Downloads\ER_Finale (6).lun 
-- * Schema: GameZone/SQL 
-- ********************************************* 
...
create table LAVORO (
     Partita_iva char(1) not null,
     CF char(1) not null,
     Data_assunzione char(1) not null,
...
alter table BUG add constraint ID_BUG_CHK
     check(exists(select * from SEGNALAZIONE
                  where SEGNALAZIONE.Id_bug = Id_bug)); 
...
alter table CASA_PRODUTTRICE add constraint ID_CASA_PRODUTTRICE_CHK
     check(exists(select * from LAVORO
                  where LAVORO.Partita_iva = Partita_iva)); 

alter table CASA_PRODUTTRICE add constraint ID_CASA_PRODUTTRICE_CHK
     check(exists(select * from HARDWARE
                  where HARDWARE.Partita_iva = Partita_iva)); 

alter table CASA_PRODUTTRICE add constraint ID_CASA_PRODUTTRICE_CHK
     check(exists(select * from SOFTWARE
                  where SOFTWARE.Partita_iva = Partita_iva)); 

alter table CHIAVI add constraint ID_CHIAVI_CHK
     check(exists(select * from RISCATTO
                  where RISCATTO.Id_chiave = Id_chiave)); 
...
alter table ETICHETTA add constraint ID_ETICHETTA_CHK
     check(exists(select * from GENERE
                  where GENERE.Nome = Nome)); 
...
alter table GIOCO add constraint ID_GIOCO_CHK
     check(exists(select * from SOFTWARE
                  where SOFTWARE.Titolo = Titolo)); 

alter table GIOCO add constraint ID_GIOCO_CHK
     check(exists(select * from GENERE
                  where GENERE.Titolo = Titolo)); 
...
alter table TIPOLOGIA add constraint ID_TIPOLOGIA_CHK
     check(exists(select * from SOFTWARE
                  where SOFTWARE.TIP_Nome = Nome)); 

alter table TRANSAZIONE add constraint ID_TRANSAZIONE_CHK
     check(exists(select * from ACQUISTO
                  where ACQUISTO.Id_transazione = Id_transazione)); 
